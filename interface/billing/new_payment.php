...
if ($mode == "new_payment" || $mode == "distribute") {
    ...
    if (trim(formData('payment_method')) == 'mpesa' && trim(formData('mpesa_phone'))) {
        require_once("../../library/Mpesa.php");
        $mpesa = new Mpesa();
        $phone = trim(formData('mpesa_phone'));
        $amount = trim(formData('payment_amount'));
        $accountReference = 'PAY' . $payment_id;

        $stkResult = $mpesa->stkPush($phone, $amount, $accountReference);

        if ($stkResult && isset($stkResult['ResponseCode']) && $stkResult['ResponseCode'] == '0') {
            // Update payment record with M-Pesa reference
            sqlStatement("UPDATE ar_session SET reference = ?, description = CONCAT(description, ' - M-Pesa Ref: ', ?) WHERE session_id = ?",
                array($stkResult['CheckoutRequestID'], $stkResult['CheckoutRequestID'], $payment_id));
        }
    }
}
